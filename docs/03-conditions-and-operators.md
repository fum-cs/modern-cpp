...
| Operator                  | Associativity | Description                          | Example                              |
|---------------------------|---------------|--------------------------------------|--------------------------------------|
| `::`                     | n/a           | Global/namespace/class scope         | `::name`, `namespace::entity`       |
| `()`                     | Left to right | Parentheses or function call         | `(expression)`, `function(params)`  |
| `[]`                     | Left to right | Array subscript                      | `array[index]`                      |
| `.` / `->`               | Left to right | Member access                        | `object.member`, `pointer->member`  |
| `++` / `--`              | Right to left | Pre/post increment/decrement         | `++x`, `x++`, `--x`, `x--`          |
| `+` / `-`                | Left to right | Addition/subtraction                 | `x + y`, `x - y`                    |
| `*` / `/` / `%`          | Left to right | Multiplication/division/modulo       | `x * y`, `x / y`, `x % y`           |
| `<<` / `>>`              | Left to right | Bitwise shift left/right             | `x << y`, `x >> y`                  |
| `<` / `<=` / `>` / `>=`  | None          | Comparison operators                 | `x < y`, `x >= y`                   |
| `==` / `!=`              | None          | Equality/inequality                  | `x == y`, `x != y`                  |
| `&&`                     | Left to right | Logical AND                          | `x && y`                            |
| `||`                     | Left to right | Logical OR                           | `x || y`                            |
| `=` / `+=` / `-=`        | Right to left | Assignment operators                 | `x = y`, `x += y`, `x -= y`         |
| `throw`                  | Right to left | Exception throw                      | `throw exception`                   |
| `,`                      | Left to right | Comma sequencing                     | `x, y`                              |
